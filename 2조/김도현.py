...
# 초기값
plt.plot(man[0][0],man[0][1],'kx')
plt.plot(man[1][0],man[1][1],'kx')
#직선의 수직
a = (man[1][1] - man[0][1])/(man[1][0]-man[0][0]) # a = (y2-y1)/(x2-x1)
m = -1/a # m = -1/a
k = ((man[0][1]+man[1][1])/2 - m * (man[0][0]+man[1][0]) /2)
while(cnt < 30):
    
    for i in range(0,100):
        if(result[i][1]-(m*result[i][0]) > k):
            plt.plot(result[i][0],result[i][1],'ro')
        elif(result[i][1]-(m*result[i][0]) < k):
